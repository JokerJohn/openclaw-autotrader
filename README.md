![Day 39 Poster](./docs/assets/challenge-poster.svg)
...
Last synced by decision / 决策触发同步时间: `2026-04-17 23:57:20 CST`
...
| Metric | Value |
| --- | --- |
| Day / 当前天数 | `39 / 30` (130.0%) |
| Starting capital / 起始资金 | `5000 HKD` |
| Current equity / 当前权益 | HKD 4,847.52 |
| Net PnL / 累计盈亏 | -HKD 152.48 |
...
- Updated / 更新时间: 2026-04-17 23:57:20 CST
- Current book / 当前组合: No live positions / 暂无公开持仓
- Floating PnL / 当前浮动盈亏: +HKD 109.50
...
- Latest gate result / 最新门槛结论: No US candidate cleared the live entry bar. The strongest name, `QCOM`, still showed score -0.88, post-fee EV -0.73%, and win probability 61.1%. / 研究链路未稳定返回，但当前最高候选 QCOM 的费后 EV 为 -0.73% ，仍低于 live 开仓门槛 0.18% ，按小账户费后纪律继续 HOLD。

## Latest Decision Basis / 最新决策依据

- Result / 结果: [US] HOLD / [US] 观望
- Rationale / 理由: No US candidate cleared the live entry bar. The strongest name, `QCOM`, still showed score -0.88, post-fee EV -0.73%, and win probability 61.1%. / 研究链路未稳定返回，但当前最高候选 QCOM 的费后 EV 为 -0.73% ，仍低于 live 开仓门槛 0.18% ，按小账户费后纪律继续 HOLD。
...
- Candidate check / 候选检查: Reviewed 5 active candidate(s). Top checks: `QCOM` (semiconductor) | score -0.88 | post-fee EV -0.73% | win 61.1%; `AAPL` (consumer hardware) | score -1.17 | post-fee EV +0.22% | win 62.6%; `XOM` (energy) | score -1.40 | post-fee EV +0.07% | win 46.8%. / 共检查 5 只活跃候选。靠前检查结果：`QCOM`（半导体） | 评分 -0.88 | 扣费后 EV -0.73% | 胜率 61.1%；`AAPL`（消费硬件） | 评分 -1.17 | 扣费后 EV +0.22% | 胜率 62.6%；`XOM`（energy） | 评分 -1.40 | 扣费后 EV +0.07% | 胜率 46.8%。
